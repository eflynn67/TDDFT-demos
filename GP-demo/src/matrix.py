...
def get_H_func(name='HO'):
    if name =='HO':
        ...
        def HO_PT(psi,psiStar,mass,alpha,q):
            '''
            Uses 2nd order finite difference scheme to construct a discretized differential 
            H operator for the GP potential.
            ...
            off_diag = np.zeros(dim)
            off_diag[1] = 1
            H = -1*(-2*np.identity(dim,dtype=complex) + np.array(sci.linalg.toeplitz(off_diag),dtype=complex))/(step_size**2) + np.diag(potentials.V_HO(grid,alpha)*(1.0j*potentials.V_HO(grid,alpha))**q)
            return H
        return HO_PT
    elif name=='quartic':
